...
// const wordssss = wordFreq(Visvar.words as string);
const wordssss = wordFreq(Visvar);
...
const fontScale = scaleLog({
    domain: [Math.min(...wordssss.map((w) => w.value)), Math.max(...wordssss.map((w) => w.value))],
    range: [5, 30],
});
                    ...
                    {(cloudWords) =>
                        cloudWords.map((w, i) => (
                            <Text
                                ...
                                key={w.text}
                                fill={wordssss[i].value >= 7 ? 'hsl(37, 100%, 44%)' : 'hsl(10, 0%, 85%)'}
                                textAnchor={'middle'}
